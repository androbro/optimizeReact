import { Button, Typography } from '@mui/material'
import { useCustomTheme } from '../../theme/ThemeProvider'
import OuterPageLayout from '../../../src/components/PageLayout'
import InnerPageLayout from '../components/PageLayout'
import { useLocation } from 'react-router-dom'

export function DesignSystem() {
    const { toggleTheme, mode } = useCustomTheme()
    const location = useLocation()

    return (
        <OuterPageLayout>
            <InnerPageLayout currentLocation={location}>
                <div className="p-8">
                    <Typography variant="h1">Design System</Typography>

                    <section className="mt-8">
                        <Typography variant="h2">Theme</Typography>
                        <Button onClick={toggleTheme}>Toggle {mode === 'light' ? 'Dark' : 'Light'} Mode</Button>
                    </section>

                    <section className="mt-8">
                        <Typography variant="h2">Colors</Typography>
                        <div className="flex gap-4 mt-4">
                            <div className="bg-primary-main p-4 text-white rounded">Primary</div>
                            <div className="bg-secondary-main p-4 text-white rounded">Secondary</div>
                        </div>
                    </section>

                    <section className="mt-8">
                        <Typography variant="h2">Typography</Typography>
                        <Typography variant="h1">Heading 1</Typography>
                        <Typography variant="h2">Heading 2</Typography>
                        <Typography variant="h3">Heading 3</Typography>
                        <Typography variant="h4">Heading 4</Typography>
                        <Typography variant="body1">Body 1 Text</Typography>
                        <Typography variant="body2">Body 2 Text</Typography>
                    </section>

                    <section className="mt-8">
                        <Typography variant="h2">Components</Typography>
                        <div className="flex gap-4 mt-4">
                            <Button variant="contained">Primary</Button>
                            <Button variant="outlined">Primary</Button>
                        </div>
                        <div className="flex gap-4 mt-4">
                            <Button variant="contained" color="secondary">
                                Secondary
                            </Button>
                            <Button variant="outlined" color="secondary">
                                Secondary
                            </Button>
                        </div>
                    </section>
                </div>
            </InnerPageLayout>
        </OuterPageLayout>
    )
}